 ...
 * @brief  Class declaration for a C++ iostream that allows blocks of data to
 *         be inserted or removed at any point in the underlying stream,
 *         shifting data around as necessary.  Data is not modified in the
...
std::cerr << "commit(): psFirst is " << this->psFirst << std::endl;
	...
	if (plenStream == -1) {
		// This is an empty stringstream in an apparently broken implementation
		this->psFirst->seekp(0, std::ios::beg);
		this->psFirst->clear();
	}
